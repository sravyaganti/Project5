...
public:
  ...
  // Copy constructor, add
  Map(const Map &other) {
      bst = (new BinarySearchTree<Pair_type, PairComp>(*other.bst));
     }
  ...
  Map &operator=(const Map &rhs) {
    ...
    delete bst;
    bst = new BinarySearchTree<Pair_type, PairComp>(*rhs.bst);
    return *this;
  }
  ...
  // Create dummy value to insert in tree and then search for it
  Value_type& operator[](const Key_type& k){
    std::pair<Key_type, Value_type> p;
    p.first = k;
    p.second=Value_type();
    Iterator ex = bst->end();
    Iterator i = bst->find(p);
    if(i==ex){
      bst->insert(p);
      i = bst->find(p);
      return i->second;
...
// Figure out how to call Iterator class's empty and size functions
private:
  // Add a BinarySearchTree private member HERE.
  // Default constructor, defines empty binary search tree with map values in dynamic memory
