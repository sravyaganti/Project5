...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    if(node == nullptr){
      ...
      Node* copy = new Node(node->datum, nullptr, nullptr);
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    else if(less(node->datum, query)==false && less(query, node->datum)==false){ //if theyre equal according to less
      return node;
    }
    else if(less(node->datum, query)==true){//if query is greater according to less
      return find_impl(node->right, query, less);
    }
    else /*if(less(query, node->datum)==true)*/{//if query is less according to less
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if(node==nullptr){
      Node* inserted = new Node(item, nullptr, nullptr);
      return inserted;
    }

    if(less(node->datum, item)==true){
      Node * rightelt = insert_impl(node->right, item, less);
      node->right = rightelt;
    }

    else {
      Node * leftelt = insert_impl(node->left, item, less);
      node->left= leftelt;
    }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    else if(less(node->right->datum, node->datum) || (less(node->datum, node->left->datum))){
      return false;
    }
    else{
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    else if(less(val, node->datum)==true){//if datum at node is greater than val
      return node;
    }
    else{
